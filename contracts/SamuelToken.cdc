...
// SamuelToken contract: Implements fungible token on Flow blockchain
pub contract SamuelToken: FungibleToken {
    ...
    pub resource Vault: FungibleToken.Provider, FungibleToken.Receiver, FungibleToken.Balance, CollectionPublic {
        ...
        pub fun deposit(from: @FungibleToken.Vault) {
            let vault <- from as! @SamuelToken.Vault
        ...
        // Destroy the vault
        destroy() {
            SamuelToken.totalSupply = SamuelToken.totalSupply - self.balance
        }
    }
    ...
    pub resource Minter {
        ...
        pub fun mintToken(amount: UFix64): @FungibleToken.Vault {
            SamuelToken.totalSupply = SamuelToken.totalSupply + amount
            return <-create Vault(balance: amount)
        }
    }
    ...
    init() {
        ...
        self.account.link<&SamuelToken.Minter>(/public/Minter, target: /storage/MinterStorage)
