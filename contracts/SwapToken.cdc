...
// SwapToken contract: Facilitates token swapping between SamuelToken and FlowToken
pub contract SwapToken {
    ...
    // Function to swap tokens between SamuelToken and FlowToken
    pub fun swapTokens(signer: AuthAccount, swapAmount: UFix64) {

        // Borrow SamuelToken and FlowToken vaults from the signer's storage
        let SamuelTokenVault = signer.borrow<&SamuelToken.Vault>(from: /storage/VaultStorage)
            ?? panic("Could not borrow SamuelToken Vault from signer")

        let flowVault = signer.borrow<&FlowToken.Vault>(from: /storage/FlowVault)
            ?? panic("Could not borrow FlowToken Vault from signer")  

        // Borrow Minter capability from SamuelToken
        let minterRef = signer.getCapability<&SamuelToken.Minter>(/public/Minter).borrow()
            ?? panic("Could not borrow reference to SamuelToken Minter")
        ...
        // Mint new SamuelTokens and deposit them to the vault
        let newSamuelTokenVault <- minterRef.mintToken(amount: mintedAmount)
        SamuelTokenVault.deposit(from: <-newSamuelTokenVault)
